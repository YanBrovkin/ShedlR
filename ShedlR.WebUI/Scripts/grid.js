(function($) {
    $.widget("ui.grid", {
        ...
        _renderValue: function(record, column, value) {
			...
			if (column.type == 'link') {
			    ...
			    if (record.Id !== undefined) {
			        var regExp = new RegExp("<a href=\"(.*?)\".*?>(.*?)</a>", "ig"); 
			        var tempLink = "";
			        value.replace(regExp, function ($1, $2, $3) {
			            tempLink = '<a href="' + $2 + "?id=" + record.Id + '">' + $3 + '</a>';
			            return '<a href="' + $2 + "?id=" + record.Id + '">' + $3 + '</a>';
			        });
			        value = tempLink;
			    }
			    else if (record.id !== undefined) {
			        var regExp = new RegExp("\href=([^\]]+)\]", "ig");
			        var matches = value.match(regExp);
			        value.replace(new RegExp("\href=([^\]]+)\]","ig"), function ($$, $1, $2) {
			            console.log("=== DEBUG === 1: " + $1 + " == 2: " + $2);
			            return '<a href="' + $1 + "?id=" + record.id + '">' + $2 + '</a>';
			        });
			    }
			    else if (record.ID !== undefined) {
			        var regExp = new RegExp("\href=([^\]]+)\]", "ig");
        ...
        _render: function() {
        	...
        	/* build table header */
            var thead = '<table class="table"><thead><tr>';
            $.each(self.options.columns, function(i, column) {
            	...
            	if (column.type == 'link') {
            	    thead += '<th data-index="action-column">' + (self._isEmpty(column.label) ? '' : column.label) + sortIndicator + '</th>';
            	}
            	else {
            ...
            if(self.total == 0) {
	            ...
	            $.each(self.records, function(i, record) {
					...
					$.each(self.options.columns, function (j, column) {
						...
						if (column.type == 'link') {
						    tbody += '<td data-index="action-link"' + self._buildClassAttr(column.cls) + '>' + value + '</td>';
						}
						else {
        ...
        _formatValue: function(value, type) {
        	...
        	if(type == 'currency') {
        	    ...
        	    value = new Date(parseInt(value.substr(6), 10)).toDateString();
            ...
            //---------------------
			    // 01/01/2015 - added by Yan Brovkin
			else if (type == 'link') {
			    value = value;
			}
        	return value;
